...
If you are unsure, try setting them in `dotspacemacs/user-config' first."
  ...
  ;; custom-file
  (setq custom-file (expand-file-name ".custom.el" (getenv "HOME")))
...
If REGION is set, it limits this change to text within the
REGION."
  ...
  ;; Set the default scroll margin to 3 lines...
  (setq-default scroll-margin 3)
  ;; ... but, in term-mode, set it to 0 lines.
  (add-hook 'term-mode-hook
            (lambda () "" nil
              (make-variable-buffer-local 'scroll-margin)
              (setq scroll-margin 0)))

  ;; Added so I can work offline without Spacemacs giving an error on startup.
  ;; https://emacs.stackexchange.com/questions/47603/how-do-i-get-spacemacs-working-on-machine-without-internet
  ;; https://emacs.stackexchange.com/questions/7653/elisp-code-to-check-for-internet-connection
  (defun internet-up-p (&optional host)
    (= 0 (call-process "ping" nil nil nil "-c" "1" "-W" "1"
                       (if host host "www.google.com"))))
  (unless (internet-up-p)
